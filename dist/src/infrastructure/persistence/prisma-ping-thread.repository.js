"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PrismaPingThreadRepository = void 0;
const common_1 = require("@nestjs/common");
const client_1 = require("@prisma/client");
let PrismaPingThreadRepository = class PrismaPingThreadRepository {
    constructor(prisma) {
        this.prisma = prisma;
    }
    async create(pingId, responderId) {
        return this.prisma.pingThread.create({ data: { pingId, responderId } });
    }
    async findByPingAndResponder(pingId, responderId) {
        return this.prisma.pingThread.findUnique({
            where: { pingId_responderId: { pingId, responderId } },
        });
    }
    async findById(threadId) {
        return this.prisma.pingThread.findUnique({ where: { id: threadId } });
    }
    async findByPingId(pingId) {
        return this.prisma.pingThread.findMany({
            where: { pingId },
            orderBy: { createdAt: 'desc' },
        });
    }
    async findByResponderId(responderId) {
        return this.prisma.pingThread.findMany({
            where: { responderId },
            orderBy: { createdAt: 'desc' },
        });
    }
};
exports.PrismaPingThreadRepository = PrismaPingThreadRepository;
exports.PrismaPingThreadRepository = PrismaPingThreadRepository = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [client_1.PrismaClient])
], PrismaPingThreadRepository);
