...
exports.StartThreadUseCase = void 0;
...
let StartThreadUseCase = class StartThreadUseCase {
    constructor(pingRepository, threadRepository, messageRepository) {
        this.pingRepository = pingRepository;
        this.threadRepository = threadRepository;
        this.messageRepository = messageRepository;
    }
    async execute(pingId, dto, responderId) {
        ...
        if (!ping.isActive(new Date())) {
            throw new common_1.ForbiddenException('Este ping ya expiró; no se pueden abrir conversaciones nuevas.');
        }
        if (ping.authorId === responderId) {
            throw new common_1.ForbiddenException('No puedes responder tu propio ping.');
        }
        const existing = await this.threadRepository.findByPingAndResponder(pingId, responderId);
        if (existing) {
            throw new common_1.ConflictException('Ya tienes una conversación abierta en este ping. Continúala en vez de iniciar otra.');
        }
...
exports.StartThreadUseCase = StartThreadUseCase;
exports.StartThreadUseCase = StartThreadUseCase = __decorate([
